...
int main() { 
  ...
  Int_t nHistos = 8;
  TString outdir = "/home/SRSUser/amoreSRS/results/120GeV/";
  TString name = "RotationBack_Residual_Position_Full120GeVHighStats_iter151_inclusive.root";
  TString outputFile = "newResiduals.root";
  TString rootfile = Form("%s%s", outdir.Data(), name.Data());
  TString rootfile2 = Form("%s%s", outdir.Data(), outputFile.Data());
  TH1F *residuals[8];
  TString histoName[8] = {"residualREF2X_152;1", "residualREF2Y_152;1", "residualREF3X_152;1", "residualREF3Y_152;1", "residualREF1X_152;1", "residualREF1Y_152;1", "residualUVA3X_152;1", "residualUVA3Y_152;1"};
  ...
  for (int i = 0; i < nHistos; ++i) {
    ...
    residuals[i] = new TH1F((histoName[i]).Data(), "", 100, -1, 1);
 // cout <<  " all keys " << file->ls() << endl;
TKey *key = file->FindKey(histoName[i]);
if (key ==0){
  cout << "!!Histogram does not exist!!" << endl;
  return 0;
}
    file->GetObject(histoName[i].Data(), residuals[i]);
if (residuals[i]){

    I2GFvalues myValues;
    myValues = I2GFmainLoop(residuals[i], 10, 7 , 1);
    gStyle->SetOptFit(111);
    cout << "histoname " << histoName[i] << " sigma " << myValues.sigma << " chi 2 " << myValues.rchi2 << endl;
residuals[i]->Draw();
    TFile *newFile = TFile::Open(rootfile2,"UPDATE");
    if (i==0) newFile->mkdir("DGResiduals");
    newFile->cd();
    gDirectory->cd("DGResiduals");
    myCanvas->Write();
    newFile->Close();
}
  }
...
//Arguments: (histo to be fit, N iterations to find peak using gaus fit, fit range param., do or do not plot fit on histo)
{
  ...
  if (ShowFit) NPeaks = s->Search(htemp, 2, "", 0.5); //opens a canvas (one time in a loop), even with:  s->Search(htemp, 2, "nodraw", 0.9);
