...
int main() { 
  //TCanvas *myCanvas = new TCanvas("c1","c1", 300,300);
  Int_t nHistos = 1;
  TString outdir = "/data/mike/ZZ48Scan/";
   TString name = "GlobalResidualSmoothCorrectedUnivProfHalfAllEvents.root";
   //   TString name = "GlobalResidualSmoothCorrectedUnivClusterSizeProfHalfAllEvents.root";
   //   TString outputFile = "DGausGlobalResidualClusterSizeAllEventsOverlay.root";
    TString outputFile = "DGausGlobalResidualAllEventsOverlay.root";
  TString rootfile = Form("%s%s", outdir.Data(), name.Data());
  TString rootfile2 = Form("%s%s", outdir.Data(), outputFile.Data());
  // TH1F *residuals[1];
  // TString histoName = {"globalCentroidMulti"};

  TString nameB = "GlobalResidualUncorAllEvents.root";
  TString rootfileB = Form("%s%s", outdir.Data(), nameB.Data());


  TFile* file = TFile::Open(rootfile);
   if (!file) return;

  TFile* file2 = TFile::Open(rootfileB);
   if (!file2) return;

   TCanvas *globalResUnivCorr = (TCanvas*) file->Get("c50");
   //  TCanvas *globalResUnivCorr = (TCanvas*) file->Get("c60");
   TCanvas *globalResUncorr = (TCanvas*) file2->Get("c80");
   TH1F *residual; TH1F *residual2;
   // for (int i = 0; i < nHistos; ++i) {
   //   residual = (TH1F*) globalResUnivCorr->GetPrimitive("globalResidual");
    residual = (TH1F*) globalResUnivCorr->GetPrimitive("globalResidualMulti");
   residual2 = (TH1F*) globalResUncorr->GetPrimitive("globalResidualUncorrected");

   cout << " getting histo !!!!!!!!" << endl;

  cout << "histo size " << residual->GetEntries() << endl;
  I2GFvalues myValues; I2GFvalues myValues2;
    myValues = I2GFmainLoop(residual, 10, 7 , 1);
    myValues2 =  I2GFmainLoop(residual2, 10, 7 , 1);
    
    // cout << " sigma " << myValues.sigma << " chi 2 " << myValues.rchi2 << endl;
   
    TFile *newFile = TFile::Open(rootfile2,"UPDATE");
        gStyle->SetOptStat(0);
   TCanvas *c1 = new TCanvas("c1","c1");
   residual->SetLineColor(1);
   residual->SetLineStyle(2);
   residual->SetLineWidth(2);
   myValues.fit_func->SetLineColor(1);
   myValues.fit_func->SetLineWidth(3);
   residual->Draw();
   residual2->SetLineColor(2);
   residual2->SetLineStyle(2);
   residual2->SetLineWidth(2);
   myValues2.fit_func->SetLineColor(2);
   myValues2.fit_func->SetLineWidth(3);
   residual2->Draw("same");

  TLegend *legX = new TLegend(0.66,0.6,0.9,0.9);
  legX->SetFillColor(0);
  legX->AddEntry(residual2, "Uncorrected Residual","pl");
  legX->AddEntry(myValues2.fit_func, "Uncorrected Residual Fit","pl");
  legX->AddEntry(residual, "Corrected Residual","pl");
  legX->AddEntry(myValues.fit_func, "Corrected Residual Fit","pl");
  legX->Draw();
...
//Arguments: (histo to be fit, N iterations to find peak using gaus fit, fit range param., do or do not plot fit on histo)
{
  ...
  else  NPeaks = s->Search(htemp, 2, "goff", 0.5);  //s->Search(htemp, 2, "nodraw", 0.9);
  ...
  if (peak_pos >= zero_bin_value  &&  peak_pos <= max_bin_value)  //Make sure that TSpectrum peak is within histo range
    {                                                             //if not, use Par initial values from Basic Histo Peak Find
      ...
      TS=1; //for cout below                        
      low_limit = peak_pos - (0.2 * abs(max_bin_value-zero_bin_value)); //peakpos-10% of full range of histo
      high_limit = peak_pos + (0.2 * abs(max_bin_value-zero_bin_value)); //peakpos+10% of full range of histo
  ...
  else
    {
      ...
      low_limit = binMaxCnt_value - (0.2 * abs(max_bin_value-zero_bin_value)); //peakpos-10% of full range of histo
      high_limit = binMaxCnt_value + (0.2 * abs(max_bin_value-zero_bin_value)); //peakpos+10% of full range of histo
  ...
  */
  ...
  for (int i=0; i< 8; i++)  //8 seems to work well, so let's keep it constant here.
    {
  ...
  if (abs(f_const-peak) < abs(f_const2-peak))
    {
      ...
      myI2GFvalues.fit_func = func3; //not needed
    }
  else
    {
      myI2GFvalues.rchi2 = f_RChi2;
