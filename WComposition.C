...
int Comp() {
  ...
  Float_t errorsFeICP[4] = {0.0024,0.0017,0.0027,0.002};
  ...
  */
  ...
  TMultiGraph *mgEDS = new TMultiGraph();
  ...
  /*
  TAxis *axFe = FeEDS->GetHistogram()->GetXaxis();
  Double_t x1Fe = axFe->GetBinLowEdge(1);
  Double_t x2Fe = axFe->GetBinUpEdge(axFe->GetNbins());
  FeEDS->GetHistogram()->GetXaxis()->Set(4,x1Fe,x2Fe);

  TAxis *axCu = CuEDS->GetHistogram()->GetXaxis();
  Double_t x1Cu = axCu->GetBinLowEdge(1);
  Double_t x2Cu = axCu->GetBinUpEdge(axCu->GetNbins());
  CuEDS->GetHistogram()->GetXaxis()->Set(4,x1Cu,x2Cu);

  TAxis *axSe = SeEDS->GetHistogram()->GetXaxis();
  Double_t x1Se = axSe->GetBinLowEdge(1);
  Double_t x2Se = axSe->GetBinUpEdge(axSe->GetNbins());
  SeEDS->GetHistogram()->GetXaxis()->Set(4,x1Se,x2Se);

  TAxis *axMo = MoEDS->GetHistogram()->GetXaxis();
  Double_t x1Mo = axMo->GetBinLowEdge(1);
  Double_t x2Mo = axMo->GetBinUpEdge(axMo->GetNbins());
  MoEDS->GetHistogram()->GetXaxis()->Set(4,x1Mo,x2Mo);

  TAxis *axW = WEDS->GetHistogram()->GetXaxis();
  Double_t x1W = axW->GetBinLowEdge(1);
  Double_t x2W = axW->GetBinUpEdge(axW->GetNbins());
  WEDS->GetHistogram()->GetXaxis()->Set(4,x1W,x2W);
  
  for (int i=0; i < 4; ++i) {
    FeEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
    CuEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
    SeEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
    MoEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
    WEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
  ...
  */
  ...
  mgEDS->SetTitle("; Sample; Weighted Composition (%)"); 
  ...
  mgEDS->GetXaxis()->CenterTitle();
  mgEDS->GetXaxis()->SetLabelSize(0.045);
  mgEDS->GetYaxis()->CenterTitle();
  //  mgEDS->GetXaxis()->SetTitleOffset(1.2);


    TAxis *ax = mgEDS->GetHistogram()->GetXaxis();
  Double_t x1 = ax->GetBinLowEdge(1);
  Double_t x2 = ax->GetBinUpEdge(ax->GetNbins());
  mgEDS->GetHistogram()->GetXaxis()->Set(5,x1,x2);
  
  for (int i=0; i < 5; ++i) {
    mgEDS->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName[i].Data());
  }
  ...
  legendX->AddEntry(FeEDS, "Fe", "pe");
  legendX->AddEntry(CuEDS, "Cu", "pe");
  legendX->AddEntry(SeEDS, "Se", "pe");
  legendX->AddEntry(MoEDS, "Mo", "pe");
  legendX->AddEntry(WEDS, "W", "pe");
  legendX->Draw();
  canvasX->SaveAs("EDSCompResults.root");

  TCanvas *canvasY = new TCanvas("cY","cY", 15,49,950,500);
  TMultiGraph *mgXRF = new TMultiGraph();
  ...
  det3->Draw();
  det4->Draw();
  mgXRF->SetTitle("; Sample; Weighted Composition (%)"); 
  ...
  mgXRF->GetXaxis()->CenterTitle();
  mgXRF->GetYaxis()->CenterTitle();
  //  mgXRF->GetXaxis()->SetTitleOffset(1.2);


  TAxis *axXRF = mgXRF->GetHistogram()->GetXaxis();
  Double_t x1XRF = axXRF->GetBinLowEdge(1);
  Double_t x2XRF = axXRF->GetBinUpEdge(axXRF->GetNbins());
  mgXRF->GetHistogram()->GetXaxis()->Set(4,x1XRF,x2XRF);
  
  for (int i=0; i < 4; ++i) {
    mgXRF->GetHistogram()->GetXaxis()->SetBinLabel(i+1,SampleName2[i].Data());
  }
  ...
  legendY->AddEntry(FeXRF, "Fe", "pe");
  legendY->AddEntry(CrXRF, "Cr", "pe");
  legendY->AddEntry(SeXRF, "Se", "pe");
  legendY->AddEntry(MoXRF, "Mo", "pe");
  legendY->AddEntry(WXRF, "W", "pe");
  legendY->Draw();
  canvasY->SaveAs("XRFCompResults.root");
